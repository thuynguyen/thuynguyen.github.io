...
;(function(window, document, fabric) {
  ...
  ButtonGroup.prototype.createButton = function(options) {
    ...
    var extra_buttons = ["rotate-left", "rotate-right", "save", "forward", "back"];
  ...
  Darkroom.prototype = {
    ...
    initDOM: function(element) {
      ...
      toolbar.innerHTML = '<ul class="darkroom-toolbar-actions"></ul><div class="group-actions">
      <div class="filter-brightness-setting filter-setting">
        <input type="range" data-filter="brightness" value="0" step="1" max="100" min="-100">
      </div><div class="filter-saturation-setting filter-setting">
        <input type="range" data-filter="saturation" value="0" step="1" max="100" min="-100">
      </div><div class="filter-clip-setting filter-setting">
        <input type="range" data-filter="red" value="0" step="1" max="100" min="-100">
        <input type="range" data-filter="green" value="0" step="1" max="100" min="-100">
        <input type="range" data-filter="blue" value="0" step="1" max="100" min="-100">
      </div></div>';
    ...
    initCamanJS: function(){

      $(".filter-brightness-setting input[type=range]").on("change", function(){
      ...
      $(".filter-saturation-setting input[type=range]").change(function(){
        ...
        Caman(".lower-canvas", "./demo/images/sample_doodle.jpg", function () {
          this.revert(true);
          this.contrast(parseInt(satuVal)).render();
        });
      });

      $(".filter-clip-setting input[data-filter='red']").change(function(){
        var self = this;
        var redVal = $(self).val();
        Caman(".lower-canvas", "./demo/images/sample_doodle.jpg", function () {
          this.revert(false);
      ...
      $(".filter-clip-setting input[data-filter='green']").change(function(){
        var self = this;
      ...
      $(".filter-clip-setting input[data-filter='blue']").change(function(){
        var self = this;
    ...
    callBrightness: function(){
      $(".darkroom-button-danger").click()
      $(".group-actions").toggle();
      if($(".group-actions").css("display") == "block"){
        $(".filter-setting").hide();
        $(".filter-brightness-setting").show();
      }
    ...
    callClip: function(){
      ...
      if($(".group-actions").css("display") == "block"){
        $(".filter-setting").hide();
        $(".filter-clip-setting").show();
      }
    },
    callSaturation: function(){
      $(".darkroom-button-danger").click()
      $(".group-actions").toggle();
      if($(".group-actions").css("display") == "block"){
        $(".filter-setting").hide();
        $(".filter-saturation-setting").show();
      }
    },
